...
A complete HTML and `.docset` documentation can be generated by running the `Documentation` scheme. You can use a popular documentation browser like [Dash](https://kapeli.com/dash) to access the `.docset` artifact or browse the HTML directly in the `Docs/Buy` and `Docs/Pay` directories.
