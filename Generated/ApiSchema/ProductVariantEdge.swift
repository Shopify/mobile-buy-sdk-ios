...
extension ApiSchema {
	open class ProductVariantEdgeQuery: GraphQL.AbstractQuery {
		...
		@discardableResult
		open func node(aliasSuffix: String? = nil, _ subfields: (ProductVariantQuery) -> Void) -> ProductVariantEdgeQuery {
	...
	open class ProductVariantEdge: GraphQL.AbstractResponse
	{
		...
		open var typeName: String { return "ProductVariantEdge" }
		...
		open var node: ApiSchema.ProductVariant {
			return internalGetNode()
		}

		func internalGetNode(aliasSuffix: String? = nil) -> ApiSchema.ProductVariant {
			return field(field: "node", aliasSuffix: aliasSuffix) as! ApiSchema.ProductVariant
		}
