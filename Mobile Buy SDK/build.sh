...
xcodebuild -scheme ${FRAMEWORK_NAME} -configuration "${BUILD_CONFIGURATION}" -sdk "iphoneos8.3" clean build ARCHS="armv7 armv7s arm64" IPHONEOS_DEPLOYMENT_TARGET="8.0" TARGET_BUILD_DIR="${DIR_ARM}" BUILT_PRODUCTS_DIR="${DIR_ARM}"
xcodebuild -scheme ${FRAMEWORK_NAME} -configuration "${BUILD_CONFIGURATION}" -sdk "iphonesimulator8.3" clean build ARCHS="x86_64" IPHONEOS_DEPLOYMENT_TARGET="8.0" TARGET_BUILD_DIR="${DIR_x8664}" BUILT_PRODUCTS_DIR="${DIR_x8664}"
