...
extension ApiSchema {
	open class CollectionEdgeQuery: GraphQL.AbstractQuery {
		...
		@discardableResult
		open func node(aliasSuffix: String? = nil, _ subfields: (CollectionQuery) -> Void) -> CollectionEdgeQuery {
	...
	open class CollectionEdge: GraphQL.AbstractResponse
	{
		...
		open var typeName: String { return "CollectionEdge" }
		...
		open var node: ApiSchema.Collection {
			return internalGetNode()
		}

		func internalGetNode(aliasSuffix: String? = nil) -> ApiSchema.Collection {
			return field(field: "node", aliasSuffix: aliasSuffix) as! ApiSchema.Collection
		}
