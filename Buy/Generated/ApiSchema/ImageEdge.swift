...
extension ApiSchema {
	open class ImageEdgeQuery: GraphQL.AbstractQuery {
		...
		@discardableResult
		open func node(aliasSuffix: String? = nil, _ subfields: (ImageQuery) -> Void) -> ImageEdgeQuery {
	...
	open class ImageEdge: GraphQL.AbstractResponse
	{
		...
		open var typeName: String { return "ImageEdge" }
		...
		open var node: ApiSchema.Image {
			return internalGetNode()
		}

		func internalGetNode(aliasSuffix: String? = nil) -> ApiSchema.Image {
			return field(field: "node", aliasSuffix: aliasSuffix) as! ApiSchema.Image
		}
