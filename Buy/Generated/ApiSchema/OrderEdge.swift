...
extension ApiSchema {
	open class OrderEdgeQuery: GraphQL.AbstractQuery {
		...
		@discardableResult
		open func node(aliasSuffix: String? = nil, _ subfields: (OrderQuery) -> Void) -> OrderEdgeQuery {
	...
	open class OrderEdge: GraphQL.AbstractResponse
	{
		...
		open var typeName: String { return "OrderEdge" }
		...
		open var node: ApiSchema.Order {
			return internalGetNode()
		}

		func internalGetNode(aliasSuffix: String? = nil) -> ApiSchema.Order {
			return field(field: "node", aliasSuffix: aliasSuffix) as! ApiSchema.Order
		}
