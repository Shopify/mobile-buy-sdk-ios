...
extension ApiSchema {
	open class CollectionConnectionQuery: GraphQL.AbstractQuery {
		...
		@discardableResult
		open func pageInfo(aliasSuffix: String? = nil, _ subfields: (PageInfoQuery) -> Void) -> CollectionConnectionQuery {
	...
	open class CollectionConnection: GraphQL.AbstractResponse
	{
		...
		open var typeName: String { return "CollectionConnection" }

		open var edges: [ApiSchema.CollectionEdge] {
			return internalGetEdges()
		}

		func internalGetEdges(aliasSuffix: String? = nil) -> [ApiSchema.CollectionEdge] {
			return field(field: "edges", aliasSuffix: aliasSuffix) as! [ApiSchema.CollectionEdge]
		}
